...
export default function ContactDetails({ contactInfo, socialLinks }) {
  return (
    <div className="gsap-contact-details space-y-6 md:space-y-8">
      <div className="gsap-reveal-item">
        <h3 className="text-xl md:text-2xl font-display font-bold text-foreground mb-4 md:mb-6">Get in Touch</h3>
      ...
      <div className="space-y-3 md:space-y-4" role="list" aria-label="Contact information">
        {contactInfo.map((item) => (
          <div key={item.label} className="gsap-reveal-item flex items-center gap-3 md:gap-4" role="listitem">
            ...
            <div>
              ...
              {item.href ? (
                <a href={item.href} className="text-foreground hover:text-primary-400 transition-colors text-sm md:text-base focus-visible:outline-none focus-visible:text-primary-400">{item.value}</a>
              ) : (
      ...
      <div className="gsap-reveal-item glass-card p-4 md:p-6" role="status" aria-live="polite">
        ...
        <p className="text-muted text-xs md:text-sm">
          Software Engineering student (Class of 2027) open to Junior Software Engineer roles in backend and full-stack development — production MERN/Laravel apps, cloud infrastructure, and CI/CD.
        </p>
      </div>
    </div>
  )
